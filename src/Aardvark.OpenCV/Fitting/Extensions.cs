...
namespace Aardvark.OpenCV
{
    public static class MatrixOpenCvExtensions
    {
        ...
        public static bool SolveLinear(this Matrix<double> A, Vector<double> b, out Vector<double> x, DecompTypes method)
        {
            ...
            using var mA = CvMat.FromPixelData(M, K, MatType.CV_64FC1, A.Array);    //: A (M*K)
            using var mX = CvMat.FromPixelData(K, N, MatType.CV_64FC1, x.Array);    //: x (K*N)
            using var mB = CvMat.FromPixelData(M, N, MatType.CV_64FC1, b.Array);    //: b (M*N)
            ...
            out Matrix<double> UT, out Matrix<double> VT, out Matrix<double> S, bool thin)
        {
            ...
            using var mM = CvMat.FromPixelData(M, N, MatType.CV_64FC1, A.Array);
            using var ms = CvMat.FromPixelData(N, 1, MatType.CV_64FC1, S.Array);
            using var mu = CvMat.FromPixelData(M, MM, MatType.CV_64FC1, UT.Array);
            using var mv = CvMat.FromPixelData(N, N, MatType.CV_64FC1, VT.Array);
            ...
            out Matrix<double> UT, out Matrix<double> S)
        {
            ...
            using var mM = CvMat.FromPixelData(M, N, MatType.CV_64FC1, A.Array);
            using var ms = CvMat.FromPixelData(N, 1, MatType.CV_64FC1, S.Array);
            using var mu = CvMat.FromPixelData(M, M, MatType.CV_64FC1, UT.Array);
            using var mv = new CvMat(N, N, MatType.CV_64FC1);
            ...
            out Matrix<double> VT, out Matrix<double> S)
        {
            ...
            using var mM = CvMat.FromPixelData(M, N, MatType.CV_64FC1, A.Array);
            using var ms = CvMat.FromPixelData(N, 1, MatType.CV_64FC1, S.Array);
            using var mu = new CvMat(M, M, MatType.CV_64FC1);
            using var mv = CvMat.FromPixelData(N, N, MatType.CV_64FC1, VT.Array);
        ...
        public static void ComputeSingularValues(this M44d M, out M44d U, out M44d V, out V4d S)
        {
            ...
            using var pM = CvMat.FromPixelData(4, 4, MatType.CV_64FC1, M.ToArray());
            using var ps = CvMat.FromPixelData(4, 1, MatType.CV_64FC1, s);
            using var pu = CvMat.FromPixelData(4, 4, MatType.CV_64FC1, u);
            using var pv = CvMat.FromPixelData(4, 4, MatType.CV_64FC1, v);
        ...
        public static void ComputeEigenValues(this M44d M, out M44d eigenVectors, out V4d eigenValues)
        {
            ...
            using var mat0 = CvMat.FromPixelData(rows, cols, MatType.CV_64FC1, M.ToArray());
            using var mat1 = CvMat.FromPixelData(rows, 1, MatType.CV_64FC1, VV);
            using var mat2 = CvMat.FromPixelData(rows, cols, MatType.CV_64FC1, EV);
