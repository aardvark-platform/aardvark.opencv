...
namespace Aardvark.OpenCV
{
    public static class PlaneFitting
    {
        ...
        /// </summary>
        /// <param name="points">Data points</param>
        /// <param name="rnd">Random generator used to select random subset. If null, an internal one with random seed will be initialized</param>
        /// <returns>The found plane</returns>
        public static Plane3d FitPlane3dLeastMedianOfSquares(this V3d[] points, IRandomUniform rnd = null)
        {
            if (points.Length < 3)
                return Plane3d.Invalid;
            ...
            var residuals = new double[points.Length - 3];
            ...
            for (int i = 0; i < numOfIterations; i++)
            {
                ...
                do
                {
                    ...
                    p0Index = rnd.UniformInt(points.Length);
                    p1Index = rnd.UniformInt(points.Length);
                    p2Index = rnd.UniformInt(points.Length);
                ...
                for (int k = 0, j = 0; k < points.Length; k++)
                    if (k != p0Index && k != p1Index && k != p2Index)
                        residuals[j++] = candidatePlane.Height(points[k]).Abs(); // calculate square after median

                var medianOfSquaredResiduals = residuals.Median().Square();
